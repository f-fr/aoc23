...
pub fn run(lines: *aoc.Lines) ![2]u64 {
    ...
    while (try lines.next()) |line| : (i += 1) {
        ...
        var it = std.mem.tokenizeAny(u8, parts[1], " ");
        while (it.next()) |n| seen[try aoc.toNum(usize, n)] = i;

        var cnt: u6 = 0;
        it = std.mem.tokenizeAny(u8, parts[2], " ");
        while (it.next()) |n| cnt += @intFromBool(seen[try aoc.toNum(usize, n)] == i);
